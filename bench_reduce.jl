...
@parallel_indices (ix, iy) inbounds = true function reduction_sum2!(Big::Array{Float64}, Small::Array{Float64})
    factor = div.(size(Big), size(Small))
    remdr = rem.(size(Big), size(Small))
    if sum(remdr) == 0
        lowx = 1 + (ix - 1) * 2
        ...
        Small[ix, iy] = sum(x -> x / prod(factor), Big[lowx:highx, lowy:highy])
    else
        error("Sizes dont match $remdr")
    end
    return
...
l = @benchmark @parallel (1:size(Small,1),1:size(Small,2)) reduction_sum!(Big, Small)
...
update_halo!(gg_big, Big)
l2= @benchmark @parallel (1:size(Small,1),1:size(Small,2)) reduction_sum2!(Big, Small)
update_halo!(gg_small, Small)
